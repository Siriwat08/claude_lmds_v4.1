/**
 * VERSION: 4.1
 * 🗂️ Service: Soft Delete & UUID Merging (NEW v4.1)
...
function softDeleteRecord(uuid) {
  ...
  var sheet = ss.getSheetByName(CONFIG.SHEET_NAME);
  var lastRow = getRealLastRow_(sheet, CONFIG.COL_NAME);
  var data    = sheet.getRange(2, 1, lastRow - 1, CONFIG.COL_MERGED_TO_UUID).getValues();
...
function resolveUUID(uuid) {
  ...
  data.forEach(function(row) {
    ...
    if (u) uuidMap[u] = { status: row[CONFIG.C_IDX.RECORD_STATUS], mergedTo: row[CONFIG.C_IDX.MERGED_TO_UUID] };
  });
...
function mergeDuplicates_UI() {
  ...
  var resMaster = ui.prompt("🔀 Merge UUID (1/2)", "กรุณาใส่ Master UUID (UUID ที่จะเก็บไว้):", ui.ButtonSet.OK_CANCEL);
  if (resMaster.getSelectedButton() !== ui.Button.OK) return;
  var masterUUID = resMaster.getResponseText().trim();

  var resDup = ui.prompt("🔀 Merge UUID (2/2)", "กรุณาใส่ Duplicate UUID (UUID ที่จะรวมเข้า Master):", ui.ButtonSet.OK_CANCEL);
  if (resDup.getSelectedButton() !== ui.Button.OK) return;
  var duplicateUUID = resDup.getResponseText().trim();

  if (!masterUUID || !duplicateUUID) { ui.alert("❌ UUID ไม่ครบ กรุณาลองใหม่ครับ"); return; }
  if (masterUUID === duplicateUUID)  { ui.alert("❌ Master และ Duplicate เป็น UUID เดียวกัน"); return; }

  var result = mergeUUIDs(masterUUID, duplicateUUID);

  if (!result.masterFound)    { ui.alert("❌ ไม่พบ Master UUID: " + masterUUID); return; }
  if (!result.duplicateFound) { ui.alert("❌ ไม่พบ Duplicate UUID: " + duplicateUUID); return; }

  if (typeof clearSearchCache === 'function') clearSearchCache();

  ui.alert("✅ Merge สำเร็จ!\n\nMaster UUID: " + masterUUID + "\nDuplicate UUID: " + duplicateUUID + "\n\nDuplicate ถูก Mark เป็น 'Merged' แล้วครับ\nข้อมูลเดิมยังอยู่ครบ ไม่มีอะไรถูกลบ");
}
