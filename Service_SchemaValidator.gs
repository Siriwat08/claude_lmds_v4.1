/**
 * VERSION: 4.1
 * 🛡️ Service: Schema Validator (NEW v4.1)
...
var SHEET_SCHEMA = {
  DATABASE: {
    ...
    requiredHeaders: {
      ...
      18: "Coord_Source", 19: "Coord_Confidence", 20: "Coord_Last_Updated"
    }
  },
  NAMEMAPPING: {
    ...
    minColumns: 5,
    requiredHeaders: {
      1: "Variant_Name", 2: "Master_UID", 3: "Confidence_Score",
      4: "Mapped_By", 5: "Timestamp"
    }
  },
  SCG_SOURCE: {
...
function validateSheet_(schemaKey) {
  ...
  if (lastCol < schema.minColumns) {
    errors.push("❌ ชีต '" + sheetName + "' มีแค่ " + lastCol + " คอลัมน์ (ต้องการ ≥ " + schema.minColumns + ")");
  }
  ...
  if (lastCol > 0 && sheet.getLastRow() > 0) {
    ...
    Object.keys(schema.requiredHeaders).forEach(function(colNum) {
      ...
      if (actual.toString().trim() !== expected.toString().trim()) {
        errors.push("⚠️ Col " + colNum + ": คาดว่า '" + expected + "' แต่เจอ '" + actual + "'");
      }
    });
  }
...
function throwSchemaError_(results, flowName) {
  ...
  SpreadsheetApp.getActiveSpreadsheet().toast("❌ Schema Error: " + flowName, "System Alert", 10);
  throw new Error(msg);
}
...
function fixNameMappingHeaders() {
  ...
  var newHeaders = ["Variant_Name","Master_UID","Confidence_Score","Mapped_By","Timestamp"];
  ...
  ui.alert("✅ อัปเกรด NameMapping Header เป็น V4.0 สำเร็จ!");
}
